...
def detect(t):
    ...
    face_big = (fh / H) > 0.18   # face-cam plein ecran = gros visage, meme immobile (peu de mouvement)
...
for s in merged:
    ...
    if len(rects) >= 3:
        ...
        s['bbox'] = [round(L / W, 4), round(T / H, 4), round((R - L) / W, 4), round((B - T) / H, 4)]
        refined += 1
print("PASS B: %d pip segments raffines (edge-scan median)" % refined)
