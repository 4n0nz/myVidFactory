...
CL=$(ls $AG/out/bande_nuit/*.webm | tr '\n' ',' | sed 's/,$//')
DUR=$(ffprobe -v error -show_entries format=duration -of csv=p=0 $VG/out/b_$ID.mp4)
$AG/.venv-sd/bin/python $AG/piste_avatar.py $AG/out/piste_nuit.mp4 $AG/out/voix_$ID.wav \
...
ls -lh $AG/out/NUIT_$ID.mp4 | awk '{print $5, $9}'
