...
echo "finalize $id : x=$X ($sx), y=$Y ($sy), duree ${TOT}s"
ffmpeg -y -v error -stream_loop -1 -i "$BG" -i "$SRC" -filter_complex \
  "[0:v]fps=30,scale=1920:1080,trim=duration=${TOT},setpts=PTS-STARTPTS[bg];[1:v]scale=1440:810,setpts=PTS-STARTPTS+1.5/TB[fg];[bg][fg]overlay=${X}:${Y}:eof_action=pass[v];[1:a]adelay=1500|1500,apad=pad_dur=1.5[a]" \
