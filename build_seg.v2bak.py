...
TOP_EXT = 0.22   # extension du haut (frac de la hauteur bbox) — la webcam depasse au-dessus du visage
SIDE    = 0.05   # marge laterale/bas
...
for si, s in enumerate(hmap):
    ...
    if host == 'hero':
        ...
        if rect:
            x, y, w, h = rect
            fc = "[1:v]%s[av];[0:v][av]overlay=%d:%d:shortest=1[vo]" % (cover(w, h), x, y)
            cmd = ('ffmpeg -y -ss %s -t %s -i %s -stream_loop -1 -i %s '
                   '-filter_complex "%s" -map "[vo]" -an -r %s -t %s %s "%s"'
                   % (ss, d, source, avatar, fc, FPS, d, NV, sf))
...
lines.append('ffmpeg -y -i %s/videoonly.mp4 -i %s -map 0:v -map 1:a -c:v copy -c:a copy -shortest "%s"'
             % (segdir, source, outp))
